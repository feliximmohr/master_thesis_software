...
class DataGenerator_raw(Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            targets_idx = np.floor(ID/(self.n_subjects*self.n_frames)) #one target row for all 100 frames for each subject 0...1999 --> t=0
            subject_idx = ID - feature_idx*self.n_subjects 
            
            target = self.target_data[targets_idx].values
            
            # Store sample
            X[i,] = self.feature_data[feature_idx].values
